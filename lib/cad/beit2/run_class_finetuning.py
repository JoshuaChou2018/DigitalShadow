...
def generate_random_port():
    return random.randint(1024, 49151)  # 一般建议使用1024到49151之间的端口
...
def main(args, ds_init):
    ...
    with open(f'{args.output_dir}/classes.txt', 'w') as w:
        w.write(', '.join(class_names))

    #######
    samples_weight = make_weights_for_balanced_classes(dataset_train.imgs, len(dataset_train.classes))
    ...
    if True: #args.distributed:
        ...
        #######
        print('use custom sampler for training and sequential sampler for validation')
...
if __name__ == '__main__':
    ...
    while not is_port_open(port=port):
        port = generate_random_port()
