...
def evaluate(data_loader, model, device):
    ...
    for step, batch in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        train_preds.extend(output.argmax(dim=1).detach().cpu().numpy())
        train_trues.extend(target.detach().cpu().numpy())

    # acc1, acc5 = accuracy(output, target, topk=(1, 5))
    acc5 = -1
    acc = accuracy_score(train_trues, train_preds)
    ...
    metric_logger.meters['acc1'].update(acc, n=1)
    metric_logger.meters['acc5'].update(acc5, n=1)
    ...
    conf_matrix = confusion_matrix(train_trues, train_preds)
    print(conf_matrix)
